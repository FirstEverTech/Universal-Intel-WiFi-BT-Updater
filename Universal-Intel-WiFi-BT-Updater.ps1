...
$githubBaseUrl = "https://raw.githubusercontent.com/FirstEverTech/Universal-Intel-WiFi-BT-Updater/main/"
$wifiDriversUrl = $githubBaseUrl + "Intel_WiFi_Drivers_Download.txt"
$bluetoothDriversUrl = $githubBaseUrl + "Intel_BT_Drivers_Download.txt"

# Temporary directory for downloads
$tempDir = "C:\Windows\Temp\IntelDrivers"

# Supported Intel Wi-Fi Hardware IDs
$supportedWiFiHWIDs = @(
    # Wi-Fi 7 (BE) devices
    "PCI\VEN_8086&DEV_272B", "PCI\VEN_8086&DEV_7740", "PCI\VEN_8086&DEV_A840", "PCI\VEN_8086&DEV_E340", "PCI\VEN_8086&DEV_E440",
    
    # Wi-Fi 6E (AX_6G) devices
    "PCI\VEN_8086&DEV_2725", "PCI\VEN_8086&DEV_51F0", "PCI\VEN_8086&DEV_51F1", "PCI\VEN_8086&DEV_54F0", 
    "PCI\VEN_8086&DEV_7A70", "PCI\VEN_8086&DEV_7AF0", "PCI\VEN_8086&DEV_7E40", "PCI\VEN_8086&DEV_7F70",
    
    # Wi-Fi 6 (AX) devices
    "PCI\VEN_8086&DEV_2723", "PCI\VEN_8086&DEV_02F0", "PCI\VEN_8086&DEV_06F0", "PCI\VEN_8086&DEV_34F0",
    "PCI\VEN_8086&DEV_3DF0", "PCI\VEN_8086&DEV_43F0", "PCI\VEN_8086&DEV_4DF0", "PCI\VEN_8086&DEV_A0F0",
    
    # Wi-Fi 5 (AC) devices
    "PCI\VEN_8086&DEV_2526", "PCI\VEN_8086&DEV_30DC", "PCI\VEN_8086&DEV_31DC", "PCI\VEN_8086&DEV_9DF0",
...
Write-Host "=== Intel Wi-Fi and Bluetooth Drivers Update ===" -ForegroundColor Cyan
...
Write-Host "Latest Wi-Fi Driver: $($wifiData.Version)" -ForegroundColor Yellow
...
# Filter to only supported devices based on HW_ID
$supportedWirelessDevices = @()
foreach ($device in $wirelessDevices) {
    $isSupported = $false
    foreach ($hwid in $supportedWiFiHWIDs) {
    ...
    if ($isSupported) {
        $supportedWirelessDevices += $device
    }
}

if ($supportedWirelessDevices.Count -eq 0) {
    Write-Host "No supported Intel Wi-Fi adapters found." -ForegroundColor Yellow
    $wifiUpdateAvailable = $false
} else {
    Write-Host "Found $($supportedWirelessDevices.Count) Intel Wireless adapter(s):" -ForegroundColor Green
    $wifiUpdateAvailable = $false
    
    foreach ($device in $supportedWirelessDevices) {
        $currentVersion = Get-CurrentDriverVersion -DeviceInstanceId $device.InstanceId
        
        Write-Host "Adapter: $($device.FriendlyName)" -ForegroundColor White
...
# Find devices by Hardware ID - and ensure it's always treated as an array
$bluetoothDevices = @(Get-PnpDevice | Where-Object {
    $device = $_
    $intelBluetoothIds | Where-Object { $device.InstanceId -like $_ }
})

# POPRAWIONY BŁĄD: Zawsze traktuj $bluetoothDevices jako tablicę i pobieraj liczbę prawidłowo
$bluetoothCount = $bluetoothDevices.Count
if ($bluetoothCount -eq 0) {
    Write-Host "No Intel Bluetooth adapters found." -ForegroundColor Yellow
    $bluetoothUpdateAvailable = $false
} else {
    Write-Host "Found $bluetoothCount Intel Bluetooth adapter(s):" -ForegroundColor Green
    $bluetoothUpdateAvailable = $false
...
# Variables to track if we're doing force reinstall
$forceReinstall = $false

# If all devices are up to date, ask if user wants to reinstall anyway
if ((-not $wifiUpdateAvailable -and -not $bluetoothUpdateAvailable) -and ($supportedWirelessDevices.Count -gt 0 -or $bluetoothCount -gt 0)) {
    Write-Host "`nAll adapters are up to date." -ForegroundColor Green
    $response = Read-Host "`nDo you want to force reinstall the driver(s) anyway? (Y/N)"
    if ($response -eq "Y" -or $response -eq "y") {
        ...
        Write-Host "Force reinstall: Drivers will be reinstalled even if versions match." -ForegroundColor Yellow
        Write-Host "Note: Wi-Fi and Bluetooth will be temporarily disconnected during update." -ForegroundColor Yellow
...
if ($wifiUpdateAvailable -or $bluetoothUpdateAvailable) {
    ...
    $response = Read-Host "Do you want to proceed with driver(s) update? (Y/N)"
} else {
    $response = "N"
}
...
if ($response -eq "Y" -or $response -eq "y") {
    ...
    # Download Wi-Fi drivers if needed
    if ($wifiUpdateAvailable -and $wifiData.CabUrl) {
        Write-Host "Downloading Wi-Fi drivers..." -ForegroundColor Green
        $wifiDriverPath = "$tempDir\WiFi"
        if (Download-Extract-Cab -CabUrl $wifiData.CabUrl -OutputPath $wifiDriverPath) {
            Write-Host "Wi-Fi drivers downloaded successfully." -ForegroundColor Green
        } else {
            Write-Host "Failed to download Wi-Fi drivers." -ForegroundColor Red
            $wifiUpdateAvailable = $false
        }
    }
        ...
        foreach ($bluetoothDriverPath in $bluetoothDriverPaths) {
            if (Test-Path $bluetoothDriverPath) {
                ...
                foreach ($infFile in $infFiles) {
                    if ($forceReinstall) {
                        # Use force reinstall method for Bluetooth - suppress output by assigning to $null
                        foreach ($device in $bluetoothDevices) {
                            $null = Force-ReinstallDriver -DeviceInstanceId $device.InstanceId -InfFilePath $infFile.FullName -DeviceType "Bluetooth"
                        }
                    } else {
                        # Normal installation
